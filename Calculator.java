...
public class Calculator {
    ...
    private Calculator() {
        ...
        btnSwitchThemes.setText("多彩主题");
        btnSwitchThemes.setBackground(new Color(254, 186, 7));
        btnSwitchThemes.setForeground(Color.WHITE);
        btnSwitchThemes.setCursor(new Cursor(Cursor.HAND_CURSOR));//Display the mouse as a hand cursor type.
        btnSwitchThemes.addActionListener((e) -> onSwitchTheme());
        window.add(btnSwitchThemes);

        btnSwitchToScientificMode = new JButton();
        btnSwitchToScientificMode.setBounds(30, 30, 140, 18);
        btnSwitchToScientificMode.setText("科学计算模式");
        btnSwitchToScientificMode.setBackground(Color.darkGray);
        ...
        int[] x = {MARGIN_X, MARGIN_X + 90, 200, 290, 380};//数组实现，省去许多变量
        int[] y = {MARGIN_Y, MARGIN_Y + 100, MARGIN_Y + 180, MARGIN_Y + 260, MARGIN_Y + 340, MARGIN_Y + 420};

        inText = new JTextField("0");
        inText.setBounds(x[0], y[0], 350, 70);
        inText.setEditable(false);//细节，用户不可更改输入文本框的内容
        inText.setBackground(Color.WHITE);
        inText.setFont(new Font("Comic Sans MS", Font.PLAIN, 33));//字体要略大一点点
        window.add(inText);
        ...
        btnDiv.addActionListener(event -> {
            ...
            if (Pattern.matches("([-]?\\d+[.]\\d*)|(\\d+)", inText.getText()))
                if (go) {
                    val = calc(val, inText.getText(), opt);
                    if (Pattern.matches("[-]?[\\d]+[.][0]*", String.valueOf(val))) {//-17.000||1000.000
                        inText.setText(String.valueOf((int) val));//减少小数点后的后缀0
                    } else {
                        inText.setText(String.valueOf(val));
                    }
                    opt = '/';
    ...
    private double calc(double x, String input, char opt) {
        ...
        //inText.setFont(inText.getFont().deriveFont(Font.PLAIN));//删除可视化效果，交互性更好，
        return y;
    }
    ...
    private void onSwitchTheme() {
        if (isToggleColorSelected) {
            btnSwitchThemes.setText("开启多彩主题");
            ...
            btnSwitchThemes.setText("关闭多彩主题");
            btnSwitchThemes.setBackground(new Color(87,149,114));
    ...
    private void onShowScientificMode() {
        ...
        if (isScientificMode) {
        	btnSwitchToScientificMode.setText("打开科学计算模式");
        	...
        	btnSwitchToScientificMode.setText("关闭科学计算模式");
